...
// ── Time formatting ─────────────────────────────────────────────────
// Server stores timestamps in two shapes:
//   - ISO-T:   '2026-05-09T16:59:30+00:00'  (qa_tasks, qa_plans, …)
//   - SQLite:  '2026-05-12 20:55:37'        (datetime() output, e.g. spans)
// The SQLite shape has no TZ marker and Safari refuses to parse it.
// `_parseUtc` normalises both: space→T, append `Z` if no offset, so
// the value is unambiguously UTC before we render in browser-local tz.
window._parseUtc = function (iso) {
  if (!iso) return null;
  let s = String(iso);
  if (!s.includes('T')) s = s.replace(' ', 'T');
  // Anything ending in a digit (no Z, no offset) is UTC by convention here.
  if (!/[zZ]$/.test(s) && !/[+-]\d{2}:?\d{2}$/.test(s)) s += 'Z';
  const d = new Date(s);
  return isNaN(d) ? null : d;
};
window.fmtLocal = function (iso) {
  const d = window._parseUtc(iso);
  if (!d) return iso || '';
