...
guards:
  ...
  require_tool:pr_post_comment: "You stopped without replying. The user can only see pr_post_comment() output. Call pr_post_comment(text=..., parent_id={comment_id}) once, then finish with done()."
---
# Trigger
...
that explicitly references prior discussion — call `pr_list_threads()`
to orient yourself, then `pr_read_thread(<id>)` to drill in. For a
