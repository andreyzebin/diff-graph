...
- `pr_comments` — реальные комментарии опубликованные через
  post_comment. По умолчанию (если assert_via пустой).
...
- `intended_concerns` — `reflect(concerns=[...])` плюс
  `spawn_agent(focus=...)` из invocations.json. Используется когда
...
все `focus` строки которые он передал в `spawn_agent`. Используется
