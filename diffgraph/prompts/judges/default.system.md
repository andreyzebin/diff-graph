...
2. Найди ЛИШНИЕ замечания — не связанные с задачей.
   `false_positives` — это **только реально лишние findings которые
   агент опубликовал**. Если таких нет — оставляй `[]`. Не используй
   этот массив как поле для нот / комментариев / резюме ("агент ничего
   не опубликовал, лишнего нет" — это пустой массив, а не запись с
   таким текстом). Иначе балл штрафуется за несуществующий FP.

3. Оцени корректность смены статуса PR.

4. Поставь `overall_score` 0.0–1.0 — он должен ОТРАЖАТЬ сигналы которые
   ты уже посчитал выше, а не противоречить им:
   - всё что ожидалось (required_comments + concern_focuses) найдено,
     `false_positives=[]`, `agent_warnings=[]` → `overall_score ≥ 0.85`.
   - часть пропущена, FP нет → 0.5–0.8 пропорционально покрытию.
   - значимые false_positives или неверный status_change_verdict
     → ≤ 0.5.
   - ничего не найдено / всё мимо → ≤ 0.2.
   Не выдавай балл который противоречит твоему собственному `verdict` /
   `required_comments` / `concern_focuses_judgement` — это самый
   частый источник несправедливых вердиктов.
