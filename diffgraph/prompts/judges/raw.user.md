...
`forbidden_comments`, описание задачи) — НЕ жёсткими правилами
«инструмент X обязан быть вызван». Твоя работа — прочитать трейс и
ВЫСОКОУРОВНЕВО, семантически, по каждому ожиданию решить, выполнено
оно или нет:

- Если `rationale` концерна говорит «cross-source — должно быть
  делегировано» — посмотри в трейсе: был ли `agent_spawn` с
  `focus`, покрывающим этот концерн? Делегировал ли агент именно
  его, или разобрал сам через `diff_*`?
- Если ожидается, что агент рефлексирует для трекинга состояния —
  есть ли `reflect` в трейсе, осмысленный ли он?
- Если задача — fan-out по N независимым входам — видно ли N
  параллельных делегаций, или агент молотил всё сам?
- Если сценарий запрещает действие (например постинг inline-
  комментов в text+delegate режиме) — убедись, что в трейсе нет
  `pr_post_comment` / `set_review_status`.

Делегация не самоцель: агент, который сам корректно разобрал
маленький когерентный концерн, не хуже того, кто делегировал.
Оценивай по сути — привёл ли путь агента к правильному результату
и совпадает ли он с тем, что описывает `rationale`. Каждое такое
суждение — отдельный высокоуровневый ассерт; учитывай их все,
выставляя `overall_score` и описывая в `summary`, какие ожидания
по поведению выполнены, а какие нет.
